...
class Location {
	public static function ip()	return retrieveInfo().query;

	public static function isp() return retrieveInfo().isp;

	public static function country() return retrieveInfo().country;

	public static function state() return retrieveInfo().regionName;

	public static function city() return retrieveInfo().city;

	public static function zip() return retrieveInfo().zip;
	...
	static function toRad(value:Float) return value * Math.PI / 180;
	...
	static function retrieveInfo():LocationInfo {
		...
		http.onData = function(data:String){result = cast Json.parse(StringTools.trim(data));}
